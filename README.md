...
Keep `SUPABASE_SERVICE_ROLE_KEY` in the backend only. If Supabase variables are omitted, the app falls back to in-memory note storage for local development. The Supabase table stores plain notes only; temporary hashes are generated by the backend response for the current demo UI.
