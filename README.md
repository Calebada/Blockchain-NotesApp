...
A full-stack notes app that uses the official Blockfrost JavaScript SDK as its Cardano blockchain provider and Supabase for persistent note storage. The backend keeps provider credentials private, queries the latest Cardano block, and records each note action's local SHA-256 proof and Cardano block anchor in transaction history.
...
Keep `SUPABASE_SERVICE_ROLE_KEY` in the backend only. If Supabase variables are omitted, the app falls back to in-memory note storage for local development. The `notes` table stores plain notes plus a nullable `deleted_at` timestamp for soft deletes. The `note_activity` table stores wallet-scoped audit entries, including the app transaction hash and the Cardano block hash and height used to anchor each action.
...
With Supabase configured, restarting the backend preserves note activity proofs and their Cardano anchor snapshots. The app transaction hash is a local proof identifier, not an on-chain Cardano transaction hash; Cardano block data is fetched through Blockfrost.
